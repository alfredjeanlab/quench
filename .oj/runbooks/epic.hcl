...
#   oj run epic "Prototype new output format" --draft
command "epic" {
  args = "<description> [--draft]"
  run  = <<-SHELL
    if [ "${args.draft}" = "true" ]; then
      wok new epic "${args.description}" -p qn -l plan:needed -l draft:needed
      oj worker start plan
      oj worker start draft
    else
      wok new epic "${args.description}" -p qn -l plan:needed -l build:needed
      oj worker start plan
...
#
# Examples:
#   oj run idea "Explore streaming output for long-running checks"
command "idea" {
  args = "<description>"
  run  = <<-SHELL
    wok new epic "${args.description}" -p qn -l plan:needed
    oj worker start plan
  SHELL
}
...
#
# Examples:
#   oj run draft qn-abc123
#   oj run draft qn-abc123 qn-def456
command "draft" {
  args = "<issues>"
  run  = <<-SHELL
    for id in ${args.issues}; do
...
queue "drafts" {
  type = "external"
  list = "wok ready -t epic,feature -l draft:needed -l plan:ready -p qn -o json"
  take = "wok start ${item.id}"
  poll = "30s"
}
