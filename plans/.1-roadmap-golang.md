...
- [ ] Spec: auto-detected when go.mod present
- [ ] Spec: default source pattern **/*.go
- [ ] Spec: default test pattern **/*_test.go
- [ ] Spec: default ignores vendor/
- [ ] Spec: detects module name from go.mod
- [ ] Spec: detects packages from directory structure
- [ ] Spec: unsafe.Pointer without // SAFETY: comment fails
- [ ] Spec: //go:linkname without // LINKNAME: comment fails
- [ ] Spec: //go:noescape without // NOESCAPE: comment fails
- [ ] Spec: //nolint without comment fails (when configured)
...
- [ ] go.mod detection
- [ ] Default source patterns (**/*.go)
- [ ] Default test patterns (**/*_test.go)
- [ ] Default ignore patterns (vendor/)
- [ ] Module name extraction from go.mod
- [ ] Package enumeration from directory structure
- [ ] Integration test: detect packages in fixtures/go-project

## Phase 457: Go Test Fixture

_Blocked by: Phase 010 (Test Fixtures) - can run in parallel_

- [ ] fixtures/go-simple/ - small Go project with go.mod, cmd/, pkg/
- [ ] fixtures/go-multi/ - multi-package Go project
- [ ] Add Go violations to fixtures/violations/
...
- [ ] Default unsafe.Pointer pattern
- [ ] Default //go:linkname pattern
- [ ] Default //go:noescape pattern
- [ ] SAFETY comment requirement for unsafe.Pointer
- [ ] LINKNAME comment requirement for //go:linkname
- [ ] NOESCAPE comment requirement for //go:noescape
- [ ] Unit tests for Go escape patterns

## Phase 465: Go Adapter - Suppress

_Blocked by: Phase 205-220 (Escapes Check)_

- [ ] //nolint detection (single linter)
- [ ] //nolint:linter1,linter2 detection (multiple)
- [ ] //nolint detection (all linters, discouraged)
...
- [ ] Lint config file detection (.golangci.yml, .golangci.yaml, .golangci.toml)
...
- [ ] `quench check` on fixtures/go-simple with no config produces useful output
- [ ] `quench check` on fixtures/go-multi detects all packages
- [ ] Go-specific escapes detected in fixtures/violations
- [ ] Snapshot tests for Go adapter output
...
- [ ] Snapshot tests for Go test runner output
...
- [ ] Snapshot tests for Go build output
