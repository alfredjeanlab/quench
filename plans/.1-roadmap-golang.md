...
- [x] go test -json ./... execution
- [x] JSON output parsing (test2json format)
- [x] Per-test timing extraction
- [x] Pass/fail status extraction
- [x] Test count metrics
- [x] Package-level aggregation
...
- [x] Build target detection from cmd/**/main.go
- [ ] go build -ldflags="-s -w" execution
- [x] Binary file size measurement
- [x] Cold build execution (go clean -cache && go build)
- [x] Cold build timing
- [x] Hot build execution (touch && go build)
- [x] Hot build timing
- [x] Per-target metrics

### Checkpoint: Go Build Metrics Complete
- [x] `quench check --ci --build` measures Go binary size and build time
...
- [x] Go profile defaults struct (escapes, suppress, policy)
- [x] Profile enum includes "go" / "golang"
- [x] Auto-detection: go.mod → golang profile
- [x] Go profile defaults application in init

## Phase 487: Go Landing the Plane

_Blocked by: Phase 1515 (Init Command - Landing the Plane)_

- [x] Go Landing the Plane checklist items:
  - `go fmt ./...`
  - `go vet ./...`
...
- [x] `quench init` on fixtures/go-simple auto-detects golang profile
- [x] `quench init --with golang` creates Go-specific config
- [ ] `quench init --with golang,claude` populates Landing the Plane with Go items
