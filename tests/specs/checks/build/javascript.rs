...
fn javascript_bundle_size_passes() {
    let result = check("build").on("javascript/build-vite").args(&["--ci"]).json().passes();
    ...
    assert!(!size.unwrap().is_empty(), "should have at least one bundle measured");
}
...
fn javascript_reports_gzip_size() {
    ...
    for key in size.keys() {
        assert!(size_gzip.contains_key(key), "size_gzip should have entry for {}", key);
    }
}
...
fn javascript_custom_targets() {
    let result =
        check("build").on("javascript/build-custom-targets").args(&["--ci"]).json().passes();
    ...
    assert!(size_obj.contains_key("dist/app.js"), "should measure configured target dist/app.js");
    assert!(
...
fn javascript_bundle_size_exceeded() {
    let result = check("build").on("javascript/build-size-exceeded").args(&["--ci"]).json().fails();
